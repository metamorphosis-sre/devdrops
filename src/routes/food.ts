...
food.get("/search", async (c) => {
  ...
  try {
    ...
    return c.json({ error: "Food search service unavailable" }, 503);
  }
});
...
food.get("/barcode/:code", async (c) => {
  ...
  try {
    ...
    return c.json({ error: "Food barcode service unavailable" }, 503);
  }
});
...
food.get("/usda/search", async (c) => {
  ...
  try {
    ...
    return c.json({ error: "USDA food search service unavailable" }, 503);
  }
});
